...
class TrackApp(QWidget):
    ...
    def execute_track_initialization(self):
        try:
            ...
            # Display detailed output in the text box
            output = 'Track Details:\n\n'
            
            for track in tracks:
                if track:
                    output += f"Track ID {track['track_id']}:\n"
                    total_measurements = len(track['measurements'])

                    for measurement, state in track['measurements']:
                        output += f"  Measurement: {measurement}, State: {state}\n"

                    total_hits = hit_counts.get(track['track_id'], 0)
                    total_misses = miss_counts.get(track['track_id'], 0)

                    output += f"  Total Hits: {total_hits}, Total Misses: {total_misses}\n"
                    output += f"  Total Measurements: {total_measurements}\n"

                    # Firm check
                    if track['track_id'] in firm_ids:
                        output += f"  Status: Firm\n"
                    else:
                        output += f"  Status: Tentative\n"
                    output += '\n'

            # Summarize track details for each track ID
            output += 'Summary of Each Track:\n\n'
            for track in tracks:
                if track:
                    output += f"Track ID {track['track_id']} Summary:\n"
                    total_measurements = len(track['measurements'])
                    state_counts = {state: 0 for state in progression_states + ['Firm']}

                    # Count the occurrences of each state
                    for _, state in track['measurements']:
                    ...
                    output += f"  Total Hits: {total_hits}\n"
                    output += f"  Total Misses: {total_misses}\n"
                    output += '\n'

            # Display track ID statuses (free/occupied)
            output += 'Track ID Statuses:\n'
